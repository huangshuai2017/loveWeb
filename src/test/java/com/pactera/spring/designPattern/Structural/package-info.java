 ...
 * 3.装饰器模式 允许对一个现有对象添加新的功能，同时又改变其功能。实现方式上就是让装饰器类实现现有对象的装饰接口，然后将
 *              被装饰类以接口的类型引入，实现接口的时候调用被装饰类的方法，同时加入要装饰的功能；在MyBatis的二级缓存功能
 *              实现上大量使用了该模式
 * 4.代理模式 一个类代表另一个类的功能，分为i静态代理和动态代理，动态代理从实现上又分为针对接口代理和不针对接口的代理
